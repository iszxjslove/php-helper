...
class Date
{
     ...
     * @param null $local timestamp to use as the baseline
     * @param string $output formatting string
     * @return array|false|mixed|string
     * @from https://github.com/kohana/ohanzee-helpers/blob/master/src/Date.php
     */
    public static function span(int $remote, $local = null, string $output = 'years,months,weeks,days,hours,minutes,seconds')
    {
        // Normalize output
        $output = strtolower(trim((string)$output));
        if (!$output) {
            // Invalid output
            return false;
        }
        // Array with the output formats
        $outputArr = preg_split('/[^a-z]+/', $output);
        // Convert the list of outputs to an associative array
        $outputArr = array_combine($outputArr, array_fill(0, count($outputArr), 0));
        // Make the output values into keys
        extract(array_flip($outputArr), EXTR_SKIP);
        if ($local === null) {
        ...
        if (isset($outputArr['years'])) {
            $timespan -= self::YEAR * ($outputArr['years'] = (int)floor($timespan / self::YEAR));
        }
        if (isset($outputArr['months'])) {
            $timespan -= self::MONTH * ($outputArr['months'] = (int)floor($timespan / self::MONTH));
        }
        if (isset($outputArr['weeks'])) {
            $timespan -= self::WEEK * ($outputArr['weeks'] = (int)floor($timespan / self::WEEK));
        }
        if (isset($outputArr['days'])) {
            $timespan -= self::DAY * ($outputArr['days'] = (int)floor($timespan / self::DAY));
        }
        if (isset($outputArr['hours'])) {
            $timespan -= self::HOUR * ($outputArr['hours'] = (int)floor($timespan / self::HOUR));
        }
        if (isset($outputArr['minutes'])) {
            $timespan -= self::MINUTE * ($outputArr['minutes'] = (int)floor($timespan / self::MINUTE));
        }
        // Seconds ago, 1
        if (isset($outputArr['seconds'])) {
            $outputArr['seconds'] = $timespan;
        }
        if (count($outputArr) === 1) {
            // Only a single outputArr was requested, return it
            return array_pop($outputArr);
        }
        // Return array
        return $outputArr;
    }
     ...
     * @param string|bool $position 时间的开始或结束，默认为begin，可选前(begin,start,first,front)，end
     * @param null|bool|string $year 基准年，默认为null，即以当前年为基准
     * @param null|bool|string $month 基准月，默认为null，即以当前月为基准
     * @param null|bool|string $day 基准天，默认为null，即以当前天为基准
     * @param null|bool|string $hour 基准小时，默认为null，即以当前年小时基准
     * @param null|bool|string $minute 基准分钟，默认为null，即以当前分钟为基准
