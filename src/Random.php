...
class Random
{
    ...
    public static function build($type = 'alnum', $len = 8)
    {
        switch ($type) {
            ...
            case 'unique':
            case 'md5':
                return md5(md5(uniqid(self::alnum(), true)), self::alnum());
            case 'encrypt':
            case 'sha1':
                return sha1(uniqid(self::alnum(), true));
            default:
                return self::alnum($len);
        }
    }
    ...
    public static function lottery($ps, $num = 1, $unique = true)
    {
        ...
        foreach ($ps as $key => $value) {
            $value = substr($value, 0, strpos($value, ".") + 6);
